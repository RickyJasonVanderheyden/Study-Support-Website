...
import {
  ...
  ChevronRight, TrendingUp, UserCheck, Award, User, Sparkles
} from 'lucide-react';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {statCards.map((card, i) => (
                    <div key={i} className="bg-white rounded-[2rem] border border-emerald-50 p-6 hover:shadow-xl hover:shadow-emerald-900/5 transition-all group overflow-hidden relative">
                        ...
                        <p className="text-4xl font-black text-[#276332] relative z-10 tracking-tight">{card.value}</p>
                            ...
                            {invitations.length > 0 ? (
                                invitations.slice(0, 3).map((inv, i) => (
                                    <div key={i} className="p-5 bg-white border border-slate-100 rounded-3xl shadow-sm space-y-4 text-left border-l-4 border-l-amber-400">
                                        <div>
                                            <p className="text-sm font-black text-[#276332]">{inv.group?.name}</p>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mt-1">Invited by: {inv.invitedBy?.name}</p>
                                        </div>
                                ...
                                <div className="py-10 text-center text-left">
